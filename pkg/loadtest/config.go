...
type Config struct {
	...
	TxInputFile          string   `json:"tx_input_file"`          // Where to read the tx file (in CSV format).
}
...
func (c Config) Validate() error {
	...
	if len(c.TxInputFile) == 0 {
		return fmt.Errorf("expected an tx input file, but found none")
	}
	return nil
}
